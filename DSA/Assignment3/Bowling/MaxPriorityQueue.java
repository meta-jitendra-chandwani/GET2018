...
public class MaxPriorityQueue implements PriorityQueue {
	...
	public void insert(Bowler bowler) {
		try {
			...
			while (index > 1
					&& this.queue[index / 2].getQuota() < this.queue[index]
							.getQuota()) {
	...
	/**
	 * Pop - Remove bowler - which have higher priority
	 */
	public Bowler pop() {
		try {
			if (this.isEmpty()) {
	...
	public void maxHeapify(int index) {
		...
		if (left <= this.lastIndex
				&& this.queue[left].getQuota() > this.queue[maximum].getQuota()) {
			maximum = left;
		}
		if (right <= this.lastIndex
				&& this.queue[right].getQuota() > this.queue[maximum]
						.getQuota()) {
			maximum = right;
		}
		if (maximum != index) {
